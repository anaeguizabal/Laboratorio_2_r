...
```{r}
select(flights,year:day, hour, origin, dest, tailnum,carrier, arr_delay, dep_delay)
...
flights %>%
  inner_join(weather, by=c("year", "month","day","hour","origin"))%>%
  group_by(precip)%>% summarise(delay=mean(dep_delay,na.rm=T))%>%
  replace_na(list(n=0))
...
flights %>%
  ...
  group_by(dest)%>% summarise(delay=mean(arr_delay,na.rm=T))%>%
  inner_join(airports, by=c("dest"="faa"))
...
flights %>%
  inner_join(planes, by="tailnum",suffix=c("_flights","star_planes"))%>%
